...
if not WEBCAM:   # import pi camera libraries if required
...
if GPIOZERO_ON:
    ...
    try:
        ...
        print("Problem importing gpiozero library")
        quit()
    ...
    if debug:
        ...
        print("-----------------------------------")
        print("Horiz pan_pin=%i  min_angle=%i max_angle=%i" %
              (pan_pin, min_x, max_x))
        print("Vert tilt_pin=%i  min_angle=%i max_angle=%i" %
              (tilt_pin, min_y, max_y))
else
    try:
        import pipan
        p = pipan.PiPan()
    except ImportError:
        print("Error importing PiPan servo library")
        quit()
